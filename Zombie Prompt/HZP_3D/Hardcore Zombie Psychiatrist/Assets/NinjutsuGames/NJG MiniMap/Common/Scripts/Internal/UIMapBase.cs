	...
	public abstract class UIMapBase : MonoBehaviour
	{
		...
		{			
			...
			if (mNextUpdate < Time.time) //heightChanged || 
			{
				...
				if(target != null) UpdateScrollPosition();
				Profiler.EndSample(); 
		...
		public Vector3 MapToWorld()
		{
            ...
            //find the position of the world map on the screeen
				Vector3 mScreen = Camera.main.WorldToScreenPoint(planeRenderer.transform.position); //[BIG] UICamera.currentCamera.WorldToScreenPoint(planeRenderer.transform.position);
            ...
            mMouse = new Vector3(mMouse.x * NJGMapBase.instance.bounds.size.x / planeRenderer.transform.localScale.x / zoom, mMouse.y, mMouse.z * NJGMapBase.instance.bounds.size.z / planeRenderer.transform.localScale.y / zoom);
            //apply paned position
            Vector3 v2 = WorldScrollPosition();
            mMouse.x = mMouse.x + v2.x;
            mMouse.z = mMouse.z + v2.z;
            return mMouse;
		}
